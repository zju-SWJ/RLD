...
data_folder = '/data/imagenet' # os.path.join(os.path.dirname(os.path.abspath(__file__)), '../../data/imagenet')
...
def get_imagenet_train_transform_aug(mean, std):
    normalize = transforms.Normalize(mean=mean, std=std)
    train_transform = transforms.Compose(
        [
            transforms.RandomResizedCrop(224),
...
def get_imagenet_dataloaders(batch_size, val_batch_size, num_workers, aug,
    mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]):
    if aug:
        train_transform = get_imagenet_train_transform_aug(mean, std)
    else:
